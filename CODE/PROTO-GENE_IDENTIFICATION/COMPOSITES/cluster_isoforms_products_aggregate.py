...
#
# This version takes TWO input files of the same format and aggregates them:
# loci that share fragments across the two files are merged, and read counts
# for matching ORFs are summed.
...
# read both files; tag each row with its source file in case it's useful,
# but mainly just concatenate them into one list
isolist = []
for infile in (infile1, infile2):
    with open(infile) as f:
        for line in f:
            if line.strip():
                isolist.append(line.rstrip('\n').split('\t'))
...
orfreadnums = [[] for _ in range(len(orftypes))]  # by type; array of arrays
...
outname = infile1 + infile2 + '_AGGREGATED_iso_collapsed'
...
for i in range(len(orftypes)):
    if orfreadnums[i]:
        ...
        print(orftypes[i], ' average reads: n/a (no ORFs)')
