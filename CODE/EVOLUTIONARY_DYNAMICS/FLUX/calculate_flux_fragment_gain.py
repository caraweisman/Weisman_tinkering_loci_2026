	...
	with open(file) as f:
		for line in f:
			...
			if rawregion not in seenregions: # add at the end of this!!!!!!!  
				...
				for hit in hits:
					newname = long_to_short_name(hit) 
					region_flux_dict[region][speciesidx][0].append(newname) 
			else: 
				for hit in hits: 
					newname = long_to_short_name(hit)
					region_flux_dict[region][speciesidx][1].append(newname) 
	speciesidx += 1
...
for region in regions: 
	...
	for taxon in speciestree: # taxa in speciestree are numbers whose positions correspond to the positions of species in species list and therefore in flux dict subarrays
		...
		for frag in focal_frags: 
			if frag not in old_focal_frags: # ie it was in every older species (frags are listed when absent from target and present in focal) 
				phylo_fluxes[region][taxonidx][0].append(frag) # first evolutionary DISappearnce. interpretation depends on phylogeny. most obviously, loss in dhel is consistent
				# with origin afterward, but loss in any other species appears to be a loss.
		for frag in target_frags:  
			if frag not in old_target_frags: # ie it wasn't in any older species (frags are listed when present in target and absent in focal) 
				phylo_fluxes[region][taxonidx][1].append(frag) # first evolutionary appearance - easier interpretation


#outfile = open('Gainedfrags_flux_numbers', 'w')
#outfile.write('#Region \t Dhel \t Dazt \t Dalg \t Dath \t Total \n')

outfile2 = open('Total_gainedfrags_flux_numbers', 'w') # adds focal/target
...
outfile = open('Gainedfrags_flux_IDs', 'w')
